...
-   For some reason, documenting the internal data objects `raw_q`, `raw_qx_suppl` and `q_item_keys` in the usual way (documenting the object name sentinel,
    e.g. `"raw_q"`) fails with the error

        Error: 'raw_q' is not an exported object from 'namespace:fokus'

...
-   Investigate and fix the lazy-loading issue that occasionally occurs between generating "internal" and "exported" data.

-   Add a more generalized version of this function as `pkgpurl::run_nopurl_rmd()`.
...
gen_pkg_data <- function(data_files = c("internal", "exported", "asciicasts")) {
  ...
  data_files <- unique(checkmate::assert_subset(data_files,
                                                choices = eval(formals()$data_files),
                                                empty.ok = FALSE))
  all_data_files <- eval(formals()$data_files)
  data_files %<>% pal::order_by(by = all_data_files)
  ...
  tmp_purled_paths <-
    ...
    purrr::map_chr(~ knitr::purl(input = .x,
                                 output = fs::file_temp(pattern = fs::path_ext_remove(fs::path_file(.x)),
                                                        ext = "R"),
                                 quiet = TRUE))
  combined_purled_path <- fs::file_temp(pattern = "data_combined.nopurl",
                                        ext = "R")
  tmp_purled_paths %>%
    purrr::map_chr(readr::read_file) %>%
    paste0(collapse = "\npkgload::unload()\ndevtools::install(quick = TRUE, quiet = TRUE, upgrade = 'never')\n\n") %>%
    readr::write_lines(file = combined_purled_path)
  
  source(file = combined_purled_path,
         encoding = "UTF-8",
         echo = FALSE)
...
raw_q_suppl <- function(ballot_date = all_ballot_dates) {
...
#'                                canton = "aargau") |>
#'   names()
raw_q_suppl_lvl_canton <- function(ballot_date = all_ballot_dates,
                                   lvl = all_lvls,
                                   canton = all_cantons) {
...
#'                              proposal_nr = 1) %$%
#'   name
raw_q_suppl_proposal <- function(ballot_date = all_ballot_dates,
                                 lvl = all_lvls,
                                 canton = all_cantons,
                                 proposal_nr = 1L) {
...
#'                                   canton = "aargau",
#'                                   proposal_nr = 2)
raw_q_suppl_proposal_name <- function(ballot_date = all_ballot_dates,
                                      lvl = all_lvls,
                                      canton = all_cantons,
                                      proposal_nr = 1L) {
  result <-
...
#'
#' @examples
#' fokus:::raw_q_suppl_arguments(ballot_date = "2018-11-25",
#'                               lvl = "cantonal",
#'                               canton = "aargau",
#'                               proposal_nr = 2)
raw_q_suppl_arguments <- function(ballot_date = all_ballot_dates,
                                  lvl = all_lvls,
                                  canton = all_cantons,
                                  proposal_nr = 1L) {
  result <-
...
#' data][raw_q_suppl_proposal].
#'
#' @inheritParams raw_q_suppl_arguments
...
#'
#' @examples
#' fokus:::raw_q_suppl_argument(ballot_date = "2018-11-25",
#'                              lvl = "cantonal",
#'                              canton = "aargau",
#'                              proposal_nr = 2,
#'                              argument_nr = 2,
#'                              side = "pro")
raw_q_suppl_argument <- function(ballot_date = all_ballot_dates,
                                 lvl = all_lvls,
                                 canton = all_cantons,
                                 proposal_nr = 1L,
                                 argument_nr = 1L,
...
#'
#' @examples
#' fokus:::raw_q_suppl_main_motives(ballot_date = "2018-11-25",
#'                                  lvl = "cantonal",
#'                                  canton = "aargau",
#'                                  proposal_nr = 2)
raw_q_suppl_main_motives <- function(ballot_date = all_ballot_dates,
                                     lvl = all_lvls,
                                     canton = all_cantons,
                                     proposal_nr = 1L) {
  result <-
...
raw_q_suppl_elections <- function(ballot_date = all_ballot_dates,
                                  lvl = all_lvls,
                                  canton = all_cantons) {
  result <-
    raw_q_suppl_lvl_canton(ballot_date = ballot_date,
...
#'                              prcd = "majoritarian") %$%
#'   n_seats
raw_q_suppl_election <- function(ballot_date = all_ballot_dates,
                                 lvl = all_lvls,
                                 canton = all_cantons,
                                 prcd = all_prcds,
                                 election_nr = 1L) {
...
#'                                   canton = "aargau",
#'                                   prcd = "majoritarian")
raw_q_suppl_election_name <- function(ballot_date = all_ballot_dates,
                                      lvl = all_lvls,
                                      canton = all_cantons,
                                      prcd = all_prcds,
                                      election_nr = 1L) {
...
#'                          canton = "aargau")
raw_q_suppl_mode <- function(ballot_date = all_ballot_dates,
                             canton = all_cantons) {
...
#'   purrr::flatten_chr()
raw_q_suppl_skill_questions <- function(ballot_date = all_ballot_dates,
                                        lvl = all_lvls,
                                        canton = all_cantons,
                                        proposal_nr = NULL) {
  lvl <- rlang::arg_match0(arg = lvl,
...
#'                                    proposal_nr = 1,
#'                                    skill_question_nr = 1)
raw_q_suppl_skill_question <- function(ballot_date = all_ballot_dates,
                                       lvl = all_lvls,
                                       canton = all_cantons,
                                       proposal_nr = NULL,
...
#' `r pal::as_md_val_list(q_item_keys$key)`
...
#'   j = 1:3
#' )
q_item_val <- function(ballot_date = all_ballot_dates,
                       canton = cantons(ballot_date),
                       branch_path,
                       v_name,
                       key = q_item_keys$key,
...
#' @family q_internal
#' @keywords internal
q_parties <- function(ballot_date = all_ballot_dates) {
...
#'                                           "custom",
#'                                           "abstain"))
q_response_option_codes <- function(types = all_response_option_types) {
...
```{r}
assert_v_names <- function(v_names,
                           as_scalar = FALSE,
                           null_ok = FALSE) {
...
-   Unfortunately, the roxygen2 `@family` tag doesn't seem to work for *internal* datasets. Thus, we manually specify the documentation links using `@seealso`
...
Predicate functions based on the [metadata](#section-fokus-metadata) and the [raw questionnaire data](raw_q.html).

### Fundamental predicates

#### DESCRIPTION

Fundamental predicate functions never fail in case of incomplete [raw questionnaire data](raw_q.html) (they do still fail in case of completely missing
...
-   Federal elections are all `canton`-dependent since FOKUS coverage in `raw_qx_suppl` is, too.
...
proposal_argument <- function(ballot_date = all_ballot_dates,
  ...
  raw_q_suppl_argument(ballot_date = ballot_date,
                       lvl = lvl,
                       canton = canton,
                       proposal_nr = proposal_nr,
...
-   Skill questions are *not* named in the supplemental date-specific FOKUS questionnaire data but simply defined in an [array of `skill_question`
    tables](https://toml.io/en/latest#array-of-tables), so we can safely rely on `n_skill_questions()` internally.
...
-   Change param `lvl` to `lvls = all_lvls` and return *named* values depending on `lvls` and `proposal_nr`, similar to `n_proposals()`?
...
Functions to process the [raw FOKUS questionnaire data](raw_q.html) and other functions to conduct a survey.
...
#' 5. Columns [`question_common`](https://rpkg.dev/fokus/articles/raw_q_schema.html#supported-keys) and
#'    [`variable_label_common`](https://rpkg.dev/fokus/articles/raw_q_schema.html#supported-keys) were complemented, i.e. made to fall back on `question_full`
...
-   Find way to mute Pandoc conversion and Git deployment output.
...
Generates personalized QR codes from polling agency's online survey participation codes.
...
-   See section *Details* in `?fokus:::read_easyvote_municipalities` for specifics about the raw data.
...
#' | `min_age` | `r v_lbl("easyvote_municipality_min_age")` |
...
#' Get variable label
#'
#' Extracts a variable's (common) label from the [questionnaire data][qx].
#'
#' If no `ballot_date` and `canton` are specified, [`variable_label_common`](https://rpkg.dev/fokus/articles/raw_q_schema.html#supported-keys) is returned,
#' otherwise [`variable_label`](https://rpkg.dev/fokus/articles/raw_q_schema.html#supported-keys). Note that `ballot_date` and `canton` either must both be
...
#' fokus::v_lbl("participation_federal")
#'
#' # otherwise `variable_label`:
#' fokus::v_lbl(v_name = "participation_federal",
...
#'              ballot_date = "2019-10-20",
#'              canton = "aargau")
v_lbl <- function(v_name,
                  ballot_date = NULL,
                  canton = NULL) {

  assert_v_names(v_names = v_name,
                 as_scalar = TRUE)
...
-   The GCP Service Account specified in the `path_gcp_service_account_key` file must have read access to private files accessed by or write access to files
    modified by any googledrive fn relying on `auth_g_drive_gcp()`. Simply use the share functionality on Google Drive to grant such access. Note that e-mails
    can't be delivered to the service account's address (e.g. `c2d-zda@r-tidyverse.iam.gserviceaccount.com`), so make sure to unset the "notify" checkmark when
    sharing.

    We've given our Service Account `c2d-zda@r-tidyverse.iam.gserviceaccount.com` write access to the whole [`fokus_aargau` folder on Google
    Drive](https://drive.google.com/drive/folders/1qDRXkioj5JmI9jPOchzjh8ovITg6LOCH), so we shouldn't have to bother with access rights in the scope of this
    project.

...
ballot_metadata <-
  qx_suppl %>%
  purrr::map_dfr(function(q_suppl) {
    
    tibble::tibble(ballot_date = q_suppl$ballot_date,
                   canton = unique(names(q_suppl$cantonal))) %>%
      dplyr::mutate(n_cantonal_proposals = canton %>% purrr::map_int(~ length(q_suppl$cantonal[[.x]]$proposal)),
                    n_cantonal_proportional_elections = canton %>% purrr::map_int(~ length(q_suppl$cantonal[[.x]]$election$proportional)),
                    n_cantonal_majoritarian_elections = canton %>% purrr::map_int(~ length(q_suppl$cantonal[[.x]]$election$majoritarian)),
                    n_federal_proposals = length(q_suppl$federal$proposal),
                    n_federal_proportional_elections = canton %>% purrr::map_int(~ length(q_suppl$federal[[.x]]$election$proportional)),
                    n_federal_majoritarian_elections = canton %>% purrr::map_int(~ length(q_suppl$federal[[.x]]$election$majoritarian)))
  })
```
...
[`data-raw/questionnaire/YYYY-MM-DD.toml`](https://gitlab.com/zdaarau/rpkgs/fokus/-/tree/master/data-raw/questionnaire/).
...
-   Since *federal* elections in Switzerland are also organized per canton (the cantons are the electoral constituencies), the relevant data always stems from
    `q_suppl_lvl_canton()` and never from `q_suppl_lvl()`.

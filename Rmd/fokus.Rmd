...
-   Since *federal* elections in Switzerland are also organized per canton (the cantons are the electoral constituencies), the relevant data always stems from
    `raw_qstnr_suppl_lvl_canton()` and never from `raw_qstnr_suppl_lvl()`.
...
-   Technically, `tidyr::unnest()` used in `expand_qstnr_tibble()` also implicitly performs integrity check 2: it ensures that all of the simultaneously
    expanded variables are of the same length (or otherwise throws an error); obviously, this isn't as useful since it doesn't tell which `variable_name`s are
    affected.
...
-   we set `shorten_var_names(max_n_char = 30L)` to account for additional `time_*` variables which will be shortened to `t_*`, i.e. add 2 additional chars to
    the original variable name (except the special-block variables which won't have `time_*` siblings and certain exceptions which will share a common `time_*`
    variable).
...
`gitlabr::gl_push_file()` [is currently broken](https://github.com/ThinkR-open/gitlabr/issues/111#issuecomment-1897728438), thus we implement a correct
replacement.
...
Predicate functions based on the `r "[metadata](#fokus-metadata)"` and the [raw questionnaire data](raw_qstnr.html).
...
-   Federal elections are all `canton`-dependent since FOKUS coverage in `raw_qstnrs_suppl` is, too.
...
#'
#' Returns a list containing the ballot types and optionally political levels that have been covered by the FOKUS survey for
...
#'
#' @examples
#' fokus::combos_ballot_types(ballot_date = "2023-06-18",
#'                            canton = "aargau")
combos_ballot_types <- function(ballot_date = pal::pkg_config_val(key = "ballot_date",
                                                                  pkg = this_pkg),
                                lvls = all_lvls,
                                canton = cantons(ballot_date),
                                ballot_types = all_ballot_types,
  ...
  if (incl_lvl) {
    ...
    result <-
      ballot_types(ballot_date = ballot_date,
                   ...
                   canton = canton) |>
      intersect(y = ballot_types) |>
      purrr::map(\(x) list(ballot_type = x))
  }
...
#'
#' Returns a list containing the political levels, proposal numbers and optionally argument sides (pro/contra) plus argument numbers for all referendum
...
-   RDS data as a raw vector cannot be fed to `readRDS()`, but instead must be [separately decompressed and then
    unserialized](https://stackoverflow.com/a/67229892/7196903).
...
Keep this fn internal?

```{r, purl = FALSE}
read_survey_data_raw <- function(ballot_date = pal::pkg_config_val(key = "ballot_date",
                                                                   pkg = this_pkg),
                                 canton = cantons(ballot_date)) {
...
-   Remove `utils::URLencode()` once [ThinkR-open/gitlabr#111](https://github.com/ThinkR-open/gitlabr/issues/111) is fixed and released.
...
-   The [gitlabr](https://thinkr-open.github.io/gitlabr/) package unfortunately neither conveniently supports HTTP HEAD requests, nor does it expose the API
    response's HTTP header values. This means we had to roll our own `private_file_hash()` to implement light-weight ("cached") requests.
...
-   Replace `push_private_file()` with `gitlabr::gl_push_file()` once [ThinkR-open/gitlabr#111](https://github.com/ThinkR-open/gitlabr/issues/111) is fixed and
    released.

NOTES:

-   We only know how to calculate the proper SHA265 sum when `from_file = TRUE`, since `digest::digest()`'s raw vector hashes differ from file hashes...
...
-   Rename to `var_desc` to comply with terminology in pkg qstnr.
...
-   Introduce param `lang` once the FOKUS qstnr is ported to pkg qstnr.
...
-   To display a comparison between original and shortened column names and their respective lengths after applying `shorten_var_names()`, use:
...
-   Improve the underlying `salim::decline_noun_de()` fn. Cases where it currently fails include:

...
#### Assemble a working directory path
...
#### Initialize option `fokus.path_wd`
...
```{r, purl = FALSE}
wd_structure <- readr::read_file(file = "data-raw/wd_structure.md")
```
...
The constants defined below serve as a kind of starting point for a successive formation of basic functionalities of this package.

These constants are created as *package-internal data*. This means if the underlying "source of truth" changes, namely the supplemental date-specific
questionnaire data files found in the FOKUS working directory under `questionnaire/YYYY-MM-DD.toml`, the code chunks below have to be manually re-run in order
for the changes to be reflected in these constants.

Note that

-   these constants are documented separately (further down in this document).

-   although they are saved as "internal data" in `R/sysdata.rda`, they are still exported later on, something not explicitly mentioned in the book [R
    Packages](https://r-pkgs.org/data.html#data-data) (and maybe a CRAN violation?).

-   the option `fokus.path_wd` must be set to create them.

-   they are only set if user=salim to avoid headaches caused by wrongly set / unset option `fokus.path_wd`.

```{r, purl = FALSE}
if (Sys.info()[["user"]] == "salim") {
  
  fokus:::init_path_wd()
  
  # FOKUS-covered ballot dates
  ballot_dates <-
    fokus:::path_wd("questionnaire") %>%
    fs::dir_ls(type = "file",
  ...
  ballot_metadata <-
    ballot_dates %>%
    purrr::map(fokus::read_q_supplemental) %>%
    purrr::map_dfr(function(q_supplemental) {
      ...
      tibble::tibble(ballot_date = q_supplemental$ballot_date,
        ...
        dplyr::mutate(n_cantonal_referendums = canton %>% purrr::map_int(~ length(q_supplemental$cantonal[[.x]]$proposal)),
                      ...
                      n_federal_referendums = length(q_supplemental$federal$proposal),
...
Unfortunately, the roxygen2 `@family` tag doesn't seem to work for datasets. Thus, we manually specify the documentation links using `@seealso`. TODO:
investigate the reason why and ideally file a bug report.
...
**Note** that `proposal_nr` is ignored when `type = "election"`.
...
-   [ ] Do not hardcode "empty votes" category regex but source it from central fn instead.

-   [ ] Overthink `min_share`! `forcats::fct_lump_min(w = )` allows to provide a weight variable, so we should *really* use that! Also it would be cool to be
    able to provide either `min_share` or `min_n` with the latter allowing to specifiy the minimum number of (weighted) cases.
...
reorder_party_fct <- function(fct,
  ...
  # ensure new levels have the right length
  if (length(new_lvls) != length(lvls)) rlang::abort("Left-right party ordering result has wrong length. Maybe the regex rules need to be updated?")
...
## Convert logical vector to Unicode symbols
...
-   [ ] Think about implementing a fn that returns the wd structure *as structured data*, i.e. a properly nested list of paths or the like. Then this print fn

...
-   For every
    ...
    The CSV files must be downloaded manually (it's futile trying to automate this since cantonal websites tend to change more often than we do surveys). The
    following download locations are currently known:
...
ballots$referendum <-
  ...
  purrr::map(\(combo) {
    ...
    if (!fs::file_exists(path_file)) {
      cli::cli_abort("No raw ballot CSV data file found under {.file {path_file}}. Please first add the necessary CSV file and run again.")
    }
    ...
    data <-
      ...
      dplyr::rename(fso_id           = BFSNR,
                    entity           = NAME,
                    ballot_date      = DATUM,
                    electorate_total = STIMMBERECHTIGTE,
                    votes_received   = EINGELANGTE,
                    votes_empty      = LEERE,
                    votes_invalid    = UNGUELTIGE,
                    votes_valid      = GUELTIGE,
                    votes_yes        = JA_STIMMEN,
                    votes_no         = NEIN_STIMMEN) |>
      # rename cols

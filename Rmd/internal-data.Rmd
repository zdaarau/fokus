...
-   asciicasts are only regenerated manually because a) their generation takes a considerable amount of time and b) the second asciicast below requires some
    manual raw q modification before regeneration.
...
```{r, purl = FALSE, include = FALSE, eval = FALSE}
# NOTE: this chunk is disabled due to the large delay it causes; to regenerate the asciicast, run the chunk manually, rebuild the package and finally re-build
#       the vignette
...
asciicast::record(script = tmp_file,
                  ...
                  startup = quote(options(cli.width = 119L)),
                  timeout = 180L,
                  typing_speed = 0L,
                  rows = 50L,
                  cols = 120L,
...
-   This the code chunk generates a **failing** example. To properly regenerate, it requires an artificial mistake to be present in the raw q. To trigger the
    same error again, add `{ print('intentional illustrative' 'error') }` at the beginning of item `first_name`'s `variable_label`.
...
q_item_subkeys <- c("true",
                    "false",
                    cantons,
                    stringr::str_remove_all(ballot_dates, "-"),
